(function () {
  ...
  async function api(path, options) {
    ...
    if (window.webkit && window.webkit.messageHandlers && window.webkit.messageHandlers.pistickAPI) {
      var requestId = String(++nextNativeRequest);
      var requestBody = settings.body || {};
      if (typeof requestBody === "string") {
        try { requestBody = JSON.parse(requestBody); }
        catch (error) { throw new Error("The app request body is invalid."); }
      }
      return new Promise(function (resolve, reject) {
        nativeRequests[requestId] = { resolve: resolve, reject: reject };
        window.webkit.messageHandlers.pistickAPI.postMessage({
          id: requestId,
          path: path,
    ...
    try {
      ...
      throw new Error("PiStick could not complete the request. Try again.");
    }
    var payload;
    try {
